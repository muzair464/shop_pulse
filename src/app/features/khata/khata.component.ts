  ...
  @if (kpi()) {
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="card p-5 hover:shadow-md transition-shadow">
        ...
        <p class="text-2xl font-bold text-red-600 tabular-nums">{{ fmt(kpi()!.total_outstanding) }}</p>
      ...
      <div class="card p-5 hover:shadow-md transition-shadow">
        ...
        <p class="text-2xl font-bold text-gray-900 tabular-nums">
          {{ kpi()!.overdue_count > 0
              ? fmt(kpi()!.total_outstanding / kpi()!.overdue_count)
              : 'PKR 0' }}
        </p>
  ...
  <div class="card overflow-hidden">
    ...
    <div class="hidden md:block overflow-x-auto">
      <table class="w-full text-sm" aria-label="Customer Khata Records">
        <thead class="bg-gray-50 border-b border-gray-100">
          <tr>
            <th class="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide w-[40%]">Customer</th>
        ...
        <tbody class="divide-y divide-gray-100">
          @if (loading() && !customers().length) {
            @for (_ of [1,2,3,4,5]; track $index) {
              <tr class="animate-pulse">
                <td class="px-4 py-3.5"><div class="flex items-center gap-3">
            ...
            <tr>
              <td colspan="5" class="px-4 py-14 text-center">
                ...
                <p class="text-sm text-gray-400">
                  @if (searchQuery) { No customers match "{{ searchQuery }}" }
                  @else { No customers yet. Add one to get started. }
                </p>
              </td>
            </tr>
          } @else {
    ...
    @if (totalPages() > 1) {
      <div class="px-4 py-3 border-t border-gray-100 flex items-center justify-between">
        ...
        <div class="flex items-center gap-1">
          ...
          <span class="px-3 py-1 text-xs text-gray-600 tabular-nums">
            {{ page() }} / {{ totalPages() }}
          </span>
          <button type="button" (click)="setPage(page()+1)" [disabled]="page()===totalPages()"
            class="p-1.5 rounded-lg border border-gray-200 text-gray-500
        ...
        @if (detailLoading()) {
          <div class="py-12 flex justify-center">
            <lucide-icon [img]="Loader2Icon" size="28"
              class="animate-spin text-gray-200" aria-hidden="true"/>
          </div>
        } @else if (!detailTransactions().length) {
          <div class="py-14 text-center text-sm text-gray-400">No transactions yet.</div>
        } @else {
      ...
      @if (modalMode()==='add_customer') {
        <form [formGroup]="addForm" (ngSubmit)="submitAddCustomer()" class="px-6 py-5 space-y-4">
          ...
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1.5">Notes</label>
            <textarea formControlName="notes" rows="2"
              placeholder="Any additional notes…" class="form-input resize-none"></textarea>
          </div>
          @if (modalError()) {
      ...
      @if (modalMode()==='record') {
        <div class="px-6 py-5 space-y-4">
          ...
          <div>
            ...
            @if (modalCustomer()) {
              <div class="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-gray-50
          ...
          <div>
            ...
            <div class="relative">
              <span class="absolute left-3 top-1/2 -translate-y-1/2 text-base
                           font-bold text-gray-400 pointer-events-none">Rs</span>
              <input type="number" [(ngModel)]="txAmount" min="1"
          ...
          <!-- Notes -->
          <div>
            <p class="text-xs font-semibold text-gray-600 mb-1.5">Notes / Description</p>
            <input type="text" [(ngModel)]="txNotes"
              placeholder="e.g. Partial payment for iPhone 13 Pro"
              class="form-input text-sm"/>
          </div>
...
export class KhataComponent implements OnInit {
  ...
  async load(): Promise<void> {
    ...
    try {
      const p = new URLSearchParams({
        page: String(this.page()), status: this.statusFilter(),
        ...(this.searchQuery.trim() ? { search: this.searchQuery.trim() } : {}),
      });
      const data = await this.api.get<{
        customers: Customer[]; total: number; kpi: KpiData;
      }>(`/api/v1/customers?${p}`);
      this.customers.set(data.customers);
      this.total.set(data.total);
      this.kpi.set(data.kpi);
  ...
  closeDetail(): void { this.detailCustomer.set(null); this.detailTransactions.set([]); }
  ...
  async voidTx(tx: KhataTransaction): Promise<void> {
    ...
    try {
      ...
      this.kpi.update(k => k ? { ...k,
        total_outstanding: Math.max(0, k.total_outstanding - (tx.tx_type === 'CREDIT' ? tx.amount : -tx.amount)),
      } : k);
      this.toast.success('Transaction voided.');
  ...
  async submitTransaction(): Promise<void> {
    ...
    try {
      ...
      void this.load();
      this.toast.success(this.txType() === 'CREDIT'
        ? `Credit of ${fmt(this.txAmount)} added.`
        : `Repayment of ${fmt(this.txAmount)} recorded.`);
      this.closeModal();
    } catch (err) {
